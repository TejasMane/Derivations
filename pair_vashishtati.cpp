...
PairVashishtati::PairVashishtati(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  restartinfo = 0;
  one_coeff = 1;
...
PairVashishtati::~PairVashishtati()
{
  if (copymode) return;
...
void PairVashishtati::compute(int eflag, int vflag)
{




  const bigint t = update->ntimestep - (t0);
  ...
  // cout << " VASHISTATI MANNNNNNNNNN"<< "\n" ;
  // cout << " VASHISTATI t0 = " << t0 << "\n" ;
  // cout << " lambda  = " << lambda << "\n" ;
...
void PairVashishtati::settings(int narg, char **arg)
{
...
void PairVashishtati::coeff(int narg, char **arg)
{
  int i,j,n;
...
void PairVashishtati::init_style()
{
...
double PairVashishtati::init_one(int i, int j)
{
...
void PairVashishtati::read_file(char *file)
{
...
void PairVashishtati::setup_params()
{
  int i,j,k,m,n;
  ...
  for (m = 0; m < nparams; m++) {
    ...
    tmp_par = params[m].lambda1; // lamda

    params[m].lam1inv = (tmp_par == 0.0) ? 0.0 : 1.0/tmp_par; // (1 / lamda)
    tmp_par = params[m].lambda4; // \xi
    params[m].lam4inv = (tmp_par == 0.0) ? 0.0 : 1.0/tmp_par; // (1 / \xi)


    params[m].zizj = params[m].zi*params[m].zj * force->qqr2e;
    // note that bigd does not have 1/2 factor
    params[m].mbigd = params[m].bigd; // (Dij/2) in paper for all three cases
    params[m].heta = params[m].bigh*params[m].eta; // heta = H * \eta
    params[m].big2b = 2.0*params[m].bigb; // 2 * B
    params[m].big6w = 6.0*params[m].bigw; // 6 * W
    ...
    params[m].vrcc3 = params[m].mbigd*params[m].rc4inv *
      exp(-params[m].lam4rc); // D * (1/r^{4}) * e^{-r/\xi}
...
void PairVashishtati::twobody(Param *param, double rsq, double &fforce,
                            int eflag, double &eng)
{

	/*

				Reference state = ideal gas: with U = 0.1 U_{rep}
				
				param->dvrc depends on pair value but is constant for a pair

				If param->dvrc is +ve it is repulsive

				If param->dvrc is -ve it is attractive
  ...
  r4inv = rinvsq*rinvsq;
  r6inv = rinvsq*r4inv;
  reta = pow(r,-param->eta); // (1/r^{\eta})
  lam1r = r*param->lam1inv; // r * (1/\lambda)
  lam4r = r*param->lam4inv; // r * (1 / \xi)
...

  {
		...
		vc2 = (param->zizj - 0.9 * lambda * param->zizj) * exp(-lam1r)/r; // Zi * Zj * (1/r) * e^{-r/\lamda}
  ...
  // cout << "param->dvrc is " << param->dvrc << "\n";
  // cout << "param->zizj is " << param->zizj << "\n";
  // cout << "param->heta*reta is " << param->heta*reta << "\n";
  // cout << "param->c0 is " << param->c0 << "\n \n \n";
  // cout << "param->vrc is " << param->vrc << "\n \n \n";




  // cout << "lambda * param->big6w is " << lambda * param->big6w<< "\n";
...

  {
		  ...
		  fforce = ((param->dvrc - 0.9 * lambda * param->dvrc)*r  // Repulsive or positibe
		        - (	4.0*vc3 + lam4r*vc3+(param->big6w - lambda * param->big6w)*r6inv
		          		 - (param->heta - 0.9 * lambda * param->heta)*reta - vc2 - lam1r*vc2
		          )
		        ) * rinvsq; // (dvrc / r) - ()/r^2


  // if (eflag) eng = (param->bigh - 0.9 * lambda * param->bigh)*reta
  //          + vc2 - vc3 - (param->bigw - lambda * param->bigw)*r6inv
  //          - r*(param->dvrc - 0.9 * lambda * param->dvrc) + param->c0;

	    if (eflag) eng = (param->bigh - 0.9 * lambda * param->bigh)*reta
         + vc2 - vc3 - (param->bigw - lambda * param->bigw)*r6inv
         - r*(param->dvrc - 0.9 * lambda * param->dvrc) + param->cut* (param->dvrc - 0.9 * lambda * param->dvrc) 
         - (param->bigh- 0.9 * lambda * param->bigh) *param->rceta +
  ...
  else 
  {
		  ...
		  fforce = ((param->dvrc - lambda * param->dvrc)*r  // Repulsive or positibe
		        - (	4.0*vc3 + lam4r*vc3+(param->big6w - lambda * param->big6w)*r6inv
		          		 - (param->heta - 0.9 * lambda * param->heta)*reta - vc2 - lam1r*vc2
		          )
		        ) * rinvsq; // (dvrc / r) - ()/r^2
	    ...
	    // if (eflag) eng = (param->bigh - 0.9 * lambda * param->bigh)*reta
     //     + vc2 - vc3 - (param->bigw - lambda * param->bigw)*r6inv
     //     - r*(param->dvrc - lambda * param->dvrc) + param->c0;


	    if (eflag) eng = (param->bigh - 0.9 * lambda * param->bigh)*reta
         + vc2 - vc3 - (param->bigw - lambda * param->bigw)*r6inv
         - r*(param->dvrc - lambda * param->dvrc) + param->cut* (param->dvrc - lambda * param->dvrc) 
         - (param->bigh- 0.9 * lambda * param->bigh) *param->rceta +
      (param->zizj - 0.9 * lambda * param->zizj) * param->rcinv *
      exp(-param->lam1rc) - (param->mbigd - lambda * param->mbigd) * param->rc4inv *
      exp(-param->lam4rc) -
      (param->bigw - lambda * param->bigw)*param->rc6inv; 
...
void PairVashishtati::threebody(Param *paramij, Param *paramik, Param *paramijk,
                       ...
                       double *fj, double *fk, int eflag, double &eng)
{
  ...
  //  MOD HERE
  facrad = (paramijk->bigb - lambda * paramijk->bigb) * facexp * pcs;
  frad1 = facrad*gsrainvsq1;
  frad2 = facrad*gsrainvsq2;

  //  MOD HERE
  facang = (paramijk->big2b - lambda * paramijk->big2b)* facexp * delcs/pcsinvsq;
...
double PairVashishtati::switch_func(double t)
{
...
double PairVashishtati::dswitch_func(double t)
{
